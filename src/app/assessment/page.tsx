...
import { ASSESSMENT_QUESTIONS, AssessmentResponse } from '@/types/assessment';
import { validateOpenEndedResponse } from '@/utils/scoring';
import { calculateAllScores, getStarRating } from '@/utils/scoring';
import { saveAssessmentSession } from '@/lib/firestore';
...
export default function AssessmentPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const scores = await calculateAllScores(responses);
      const starRating = getStarRating(scores.overallScore);
      ...
      try {
        const feedbackResponse = await fetch('https://generatefeedback-ix3v2eesbq-uc.a.run.app', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            responses: responses.map(r => ({
              ...
              questionText: ASSESSMENT_QUESTIONS.find(q => q.id === r.questionId)?.text || '',
              response: r.response,
              category: r.category,
            })),
            scores: {
        ...
        if (feedbackResponse.ok) {
          ...
          geminiFeedback = feedbackData;
        } else {
          console.error('AI Feedback API error:', feedbackResponse.status, feedbackResponse.statusText);
        }
      } catch (error) {
      ...
      await saveAssessmentSession(
        ...
        categoryBreakdown,
        geminiFeedback,
        undefined // userId - will be added when we implement auth
      );
      ...
      setError('There was an error saving your assessment. Please try again.');
    } finally {
